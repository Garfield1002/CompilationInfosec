...
let get_reg st r f = f (Array.get st.regs r)
...
let exec_ltl_instr oc ip st : int option res =
  ...
  | exception Invalid_argument _ ->
      Error
        (Format.sprintf
           "Could not find next instruction to execute at ip=%d [in \
            exec_ltl_instr]"
           ip)
  | LAddi (rd, rs, i) ->
  ...
  | LStore (rt, i, rs, sz) ->
      ...
      get_reg st rs $ fun vs ->
      Mem.write_bytes st.mem (vt + i) (split_bytes (size_of_mas sz) vs)
      >>= fun _ -> next ip
  | LLoad (rd, rt, i, sz) ->
      get_reg st rt $ fun vt ->
  ...
  | LJmpr r -> get_reg st reg_ra $ fun ra -> OK (Some ra)
  | LConst (rd, i) ->
  ...
  | LBranch (cmp, rs1, rs2, s) ->
      ...
      if b then find_label st.labels s >>= fun n -> OK (Some n) else next ip
  | LCall callee_name -> (
      match Hashtbl.find_option st.funs callee_name with
      | Some { funstart } ->
          Array.set st.regs reg_ra (ip + 1);
          OK (Some funstart)
      | None ->
          do_builtin oc st.mem callee_name
            (list_ints_desc number_of_arguments_passed_in_registers
            |> List.rev
            |> List.map (fun i -> i + starting_arg_register)
            |> List.map (fun i -> Array.get st.regs i))
          >>= fun v ->
          (match v with None -> () | Some v -> Array.set st.regs reg_ret v);
          next ip)
  | LHalt -> OK None
  ...
  List.iteri
    (fun i p ->
      if i >= number_of_arguments_passed_in_registers then (
        ...
        Array.set regs reg_sp sp;
        Mem.write_bytes mem sp (split_bytes (Archi.wordsize ()) p) >>! ignore)
      else Array.set regs (starting_arg_register + i) p)
    params;
