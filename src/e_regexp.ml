...
let char_range (l : char list) = Charset (Set.of_list l)
...
let rec string_of_regexp r =
  ...
  | Alt (r1, r2) ->
      Printf.sprintf "(%s)|(%s)" (string_of_regexp r1) (string_of_regexp r2)
  | Cat (r1, r2) ->
      Printf.sprintf "(%s).(%s)" (string_of_regexp r1) (string_of_regexp r2)
...
let alphabet =
  char_list_of_string
    (lowercase_letters ^ uppercase_letters ^ digits ^ other_characters)
...
let identifier_material =
  char_range
    (char_list_of_string (uppercase_letters ^ lowercase_letters ^ digits ^ "_"))
...
let list_regexp : (regexp * (string -> token option)) list =
  [
    ...
    (keyword_regexp "!", fun s -> Some SYM_BOOL_NOT);
    ...
    ( Cat
        ( keyword_regexp "//",
          Cat
            ( Star (char_range (List.filter (fun c -> c <> '\n') alphabet)),
              Alt (char_regexp '\n', Eps) ) ),
      fun s -> None );
    ( Cat
        ( keyword_regexp "/*",
    ...
    ( Cat
        ( char_regexp '\'',
          Cat
            ( char_regexp '\\',
              Cat (char_range (char_list_of_string "\\tn0'"), char_regexp '\'')
            ) ),
      fun s ->
        ...
        ( char_regexp '"',
          Cat
            ( Star
                (Alt
                   ( char_range
                       (List.filter (fun c -> c <> '"' && c <> '\\') alphabet),
                     Cat
                       ( char_regexp '\\',
                         char_range (char_list_of_string "tn0\\\"") ) )),
              char_regexp '"' ) ),
      fun s ->
        Some
          (SYM_STRING
             (Stdlib.Scanf.unescaped (String.slice ~first:1 ~last:(-1) s))) );
