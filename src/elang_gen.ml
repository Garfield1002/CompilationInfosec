...
let rec make_eexpr_of_ast (a : tree) : expr res =
  let res =
    ...
    | Node (t, [ e1; e2 ]) when tag_is_binop t ->
        ...
        make_eexpr_of_ast e2 >>= fun a2 -> OK (Ebinop (binop_of_tag t, a1, a2))
    | Node (Tneg, [ e1 ]) ->
        make_eexpr_of_ast e1 >>= fun a1 -> OK (Eunop (Eneg, a1))
    | Node (Tint, [ IntLeaf i ]) -> OK (Eint i)
    | StringLeaf s -> OK (Evar s)
    | Node (Tcall, [ StringLeaf funName; Node (Targs, l) ]) ->
        Utils.list_map_res make_eexpr_of_ast l >>= fun l ->
        OK (Ecall (funName, l))
    | _ ->
        Error
...
let rec make_einstr_of_ast (a : tree) : instr res =
  let res =
    ...
    | Node (Tassign, [ StringLeaf s; e ]) ->
        make_eexpr_of_ast e >>= fun a -> OK (Iassign (s, a))
    | Node (Tif, [ e; i; NullLeaf ]) ->
        (* if without an else *)
        make_eexpr_of_ast e >>= fun a ->
        make_einstr_of_ast i >>= fun j -> OK (Iif (a, j, Iblock []))
    | Node (Tif, [ e; i1; i2 ]) ->
        make_eexpr_of_ast e >>= fun a ->
        make_einstr_of_ast i1 >>= fun j1 ->
        make_einstr_of_ast i2 >>= fun j2 -> OK (Iif (a, j1, j2))
    | Node (Twhile, [ e; i ]) ->
        make_eexpr_of_ast e >>= fun a ->
        make_einstr_of_ast i >>= fun j -> OK (Iwhile (a, j))
    | Node (Treturn, [ e ]) -> make_eexpr_of_ast e >>= fun a -> OK (Ireturn a)
    | Node (Tprint, [ e ]) -> make_eexpr_of_ast e >>= fun a -> OK (Iprint a)
    | Node (Tblock, l) ->
        Utils.list_map_res make_einstr_of_ast l >>= fun ll -> OK (Iblock ll)
    | Node (Tcall, [ StringLeaf funName; Node (Targs, l) ]) ->
        Utils.list_map_res make_eexpr_of_ast l >>= fun l ->
        OK (Icall (funName, l))
    | _ ->
...
let make_ident (a : tree) : string res =
  match a with
  | Node (Targ, [ s ]) -> OK (string_of_stringleaf s)
  | a ->
...
let make_eprog_of_ast (a : tree) : eprog res =
  ...
  | Node (Tlistglobdef, l) ->
      list_map_res
        (fun a ->
          make_fundef_of_ast a >>= fun (fname, efun) -> OK (fname, Gfun efun))
        l
  | _ ->
      Error
