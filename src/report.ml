...
let add_to_report id title content =
  report :=
    !report @ [ { sect_id = id; sect_title = title; sect_content = content } ]
...
let make_report filename report () =
  ...
  List.iter
    (fun { sect_id; sect_title; sect_content } ->
      Printf.fprintf html
        "<fieldset><h3 id=\"%s\"><a href=\"#top\">&uarr;</a> \
         %s</h3>%a</fieldset>\n"
...
let call_dot report_sectid report_secttitle file () : unit =
  if not !Options.no_dot then (
    ...
    add_to_report report_sectid report_secttitle
      (Img (Filename.basename file ^ ".svg"));
    ignore r)
...
type compile_result = { step : string; error : string option; data : Yojson.t }
type result = RunRes of run_result | CompRes of compile_result

let results : result list ref = ref []

let record_compile_result ?(error = None) ?(data = []) step =
...
let run_exn_to_error f x = try f x with e -> Error (Printexc.to_string e)
...
let timeout (f : 'a -> 'b res) (arg : 'a) (time : float) : ('b * string) res =
  ...
  | 0 ->
      let r =
        run_exn_to_error f arg >>= fun v -> OK (v, Format.flush_str_formatter ())
      in
  ...
  | pid0 -> (
      ...
      | 0 ->
          ...
          Marshal.to_channel oc
            (Error (Printf.sprintf "Timeout after %f seconds." time))
            [];
          close_out oc;
          exit 0
      | pid1 ->
  ...
  if flag then (
    ...
    let res =
      timeout
        ...
        (p, !params) !Options.timeout
    in
    let timerun = Unix.gettimeofday () -. starttime in
    let rres =
      { step; retval = None; output = ""; error = None; time = timerun }
    in
    let rres =
      match res with
    ...
    add_to_report step ("Run " ^ step)
      (Paragraph
         ...
         ^ Printf.sprintf
             "Output : <pre style=\"padding: 1em; background-color: \
              #ccc;\">%s</pre>\n"
             rres.output
         ^ (match rres.error with
           | Some msg ->
               Printf.sprintf
                 "Error : <pre style=\"padding: 1em; background-color: #fcc;\">\n\
                  %s</pre>\n"
                 msg
           | _ -> "")
...
let json_output_string () =
  ...
  let jstring_of_ostring o =
    match o with None -> `Null | Some s -> `String s
  in
  let j =
    `List
      (List.map
           ...
           | RunRes { step; retval; output; error; time } ->
               `Assoc
                 [
                   ("runstep", `String step);
                   ( "retval",
                     match retval with Some r -> `Int r | None -> `Null );
