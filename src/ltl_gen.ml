...
let arg_registers = range ~start:starting_arg_register 8
...
let make_pop r =
  [
    LLoad (r, reg_sp, 0, archi_mas ()); LAddi (reg_sp, reg_sp, Archi.wordsize ());
  ]

let make_sp_sub v = [ LSubi (reg_sp, reg_sp, v) ]
let make_sp_add v = [ LAddi (reg_sp, reg_sp, v) ]
...
let make_loc_mov src dst =
  ...
  | Reg rsrc, Reg rdst -> [ LMov (rdst, rsrc) ]
...
let load_loc tmp allocation r =
  ...
  | None -> Error (Format.sprintf "Unable to allocate RTL register r%d." r)
  | Some (Stk o) ->
      OK ([ LLoad (tmp, reg_fp, Archi.wordsize () * o, archi_mas ()) ], tmp)
  | Some (Reg r) -> OK ([], r)
...
let store_loc tmp allocation r =
  ...
  | None -> Error (Format.sprintf "Unable to allocate RTL register r%d." r)
  | Some (Stk o) ->
      OK ([ LStore (reg_fp, Archi.wordsize () * o, tmp, archi_mas ()) ], tmp)
  | Some (Reg r) -> OK ([], r)
...
let restore_caller_save arg_saved =
  List.map
    (fun (reg, ofs) ->
      LLoad (reg, reg_fp, Archi.wordsize () * ofs, archi_mas ()))
    arg_saved
...
let overwritten_args rargs allocation =
  ...
  list_map_res
    (fun r ->
      ...
      | None ->
          Error
            (Format.sprintf "overwritten_args: Couldn't allocate register r%d."
               r)
      | Some loc -> OK loc)
    rargs
  >>= fun ltl_args ->
  let overwritten, read_overwritten =
    List.fold_lefti
      (fun (overwritten, read_overwritten) i (src : loc) ->
        ...
        let read_overwritten =
          match src with
          | Reg rs ->
              if Set.mem rs overwritten then Set.add rs read_overwritten
              else read_overwritten
          | Stk _ -> read_overwritten
        in
        let overwritten =
          if i < number_of_arguments_passed_in_registers then
            Set.add (starting_arg_register + i) overwritten
          else overwritten
        in
        (overwritten, read_overwritten))
      (Set.empty, Set.empty) ltl_args
  in
  OK read_overwritten
...
let pass_parameters rargs allocation arg_saved =
  ...
  list_map_res
    (fun r ->
      ...
      | None ->
          Error
            (Format.sprintf "pass_parameters: Couldn't allocate register r%d." r)
      | Some loc -> OK loc)
    rargs
  >>= fun ltl_args ->
  ...
  let reloc_loc overwritten loc =
    ...
    | Stk o -> OK loc
    | Reg r ->
        if List.mem r overwritten then
          match List.assoc_opt r arg_saved with
          | None ->
              Error
                (Format.sprintf
                   "Register %s has been overwritten, yet it has not been \
                    saved."
                   (print_reg r))
  ...
  List.fold_lefti
    (fun acc i (src : loc) ->
      ...
      let overwritten, l, pushes, npush =
        if i < number_of_arguments_passed_in_registers then
          let rd = starting_arg_register + i in
          match src with
    ...
    (OK ([], [], [], 0))
    ltl_args
  >>= fun (overwritten, instrs, pushes, npush) -> OK (instrs @ pushes, npush)
...
let read_rtl_regs (l : rtl_instr list) =
  List.fold_left
    (fun acc i -> Set.union acc (read_rtl_regs_instr i))
    Set.empty l
...
let ltl_instrs_of_linear_instr fname live_out allocation numspilled
  ...
  let res =
    ...
    | Rprint r ->
        let save_a_regs, arg_saved, ofs =
          save_caller_save (range 32) (-(numspilled + 1))
        in
        let parameter_passing =
          match Hashtbl.find_option allocation r with
          | None ->
              Error
                (Format.sprintf "Could not find allocation for register %d\n" r)
          | Some (Reg rs) -> OK [ LMov (reg_a0, rs) ]
          | Some (Stk o) ->
              OK [ LLoad (reg_a0, reg_fp, Archi.wordsize () * o, archi_mas ()) ]
        in
        parameter_passing >>= fun parameter_passing ->
        OK
          ((LComment "Saving a0-a7,t0-t6" :: save_a_regs)
          @ LAddi (reg_sp, reg_s0, Archi.wordsize () * (ofs + 1))
            :: parameter_passing
          @ LCall "print" :: LComment "Restoring a0-a7,t0-t6"
    ...
    | Rcall (None, fname, rargs) ->
        ...
        OK
          ...
          @ LAddi (reg_sp, reg_sp, (ofs + 1) * Archi.wordsize ())
  ...
  OK
    (LComment
       (Format.asprintf
          "#<span style=\"background: pink;\"><b>Linear instr</b>: %a #</span>"
          (Rtl_print.dump_rtl_instr fname (None, None))
          ins)
    :: l)
...
let retrieve_nth_arg n numcalleesave =
  ...
  if n < thr then Reg (n + starting_arg_register)
...
let ltl_fun_of_linear_fun linprog
  ...
  let callee_saved_regs =
    Set.intersect (Set.of_list callee_saved) written_regs
  in
  List.iteri
    (fun i pr ->
  ...
  let max_label =
    List.fold_left
      (fun acc i -> match i with Rlabel l -> Stdlib.max l acc | _ -> acc)
      0 linearfunbody
  in
  ...
  let epilogue =
    ...
    :: LMov (reg_sp, reg_fp)
    :: List.concat
         (List.map make_pop (List.rev (Set.to_list callee_saved_regs)))
    @ [ LJmpr reg_ra ]
  in
  list_map_resi
    (fun i ->
      ...
      (function
        | fname, Gfun f ->
            let f_alloc =
              ...
              | Some (rig, allocation, next_stack_slot) ->
                  (allocation, -next_stack_slot - 1)
            in
            let f_lives =
...
let pass_ltl_gen linear =
  ...
  | Error msg ->
      record_compile_result ~error:(Some msg) "LTL";
      Error msg
  | OK ltl ->
      record_compile_result "LTL";
      dump !ltl_dump dump_ltl_prog ltl (fun file () ->
          add_to_report "ltl" "LTL" (Code (file_contents file)));
      OK ltl
