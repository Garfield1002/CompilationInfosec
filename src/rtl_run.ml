...
let rec exec_rtl_instr oc rp rtlfunname f st (i : rtl_instr) =
  ...
  | Rbranch (cmp, r1, r2, s1) -> (
      ...
      | Some v1, Some v2 ->
          if eval_rtl_cmp cmp v1 v2 then
            exec_rtl_instr_at oc rp rtlfunname f st s1
          else OK (None, st)
      | _, _ ->
          Error
  ...
  | Rcall (Some rd, fname, params) ->
      ...
      exec_rtl_fun oc rp st fname f params >>= fun (v, st) ->
...
and exec_rtl_instrs oc rp rtlfunname f st l =
  List.fold_left
    (fun acc i ->
      ...
      | OK (None, st) -> exec_rtl_instr oc rp rtlfunname f st i)
    (OK (None, st))
    l
...
and exec_rtl_prog oc rp memsize params =
  ...
  exec_rtl_fun oc rp st "main" f params >>= fun (v, st) -> OK v
