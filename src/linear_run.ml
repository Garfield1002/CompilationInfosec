...
let rec exec_linear_instr oc lp fname f st (i : rtl_instr) =
  ...
  | Rbranch (cmp, r1, r2, s1) -> (
      ...
      | Some v1, Some v2 ->
          if eval_rtl_cmp cmp v1 v2 then
            exec_linear_instr_at oc lp fname f st s1
          else OK (None, st)
      | _, _ ->
          Error
  ...
  | Rcall (Some rd, fname, params) ->
      ...
      exec_linear_fun oc lp st fname f params >>= fun (v, st) ->
  ...
  | Rcall (None, fname, params) ->
      let params = List.filter_map (Hashtbl.find_option st.regs) params in
      find_function lp fname >>= fun f ->
      exec_linear_fun oc lp st fname f params >>= fun (v, _) -> OK (None, st)
...
and exec_linear_instrs oc lp fname f st l =
  List.fold_left
    (fun acc i ->
      ...
      | OK (None, st) -> exec_linear_instr oc lp fname f st i)
    (OK (None, st))
    l
...
and exec_linear_prog oc lp memsize params =
  ...
  exec_linear_fun oc lp st "main" f params >>= fun (v, st) -> OK v
