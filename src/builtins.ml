...
let dump_int oc i = Format.fprintf oc "%d, " i
let dump_list pp oc l = List.iter (pp oc) l

let rec read_chars_rec mem addr chars_read =
  Mem.read_char mem addr >>= fun v ->
  if v = 0 then OK (rev chars_read)
...
let is_builtin fname =
  match fname with
  | "print" | "print_int" | "print_char" | "print_string" | "atoi" | "dump_mem"
  | "__muldi3" | "__udivdi3" | "__umoddi3" | "error" ->
      true
  | _ -> false
...
let rec do_builtin oc mem fname vargs =
  ...
  | "dump_mem", arg :: _ ->
      ...
      Format.fprintf oc "MEM :-)\n";
      OK None
  | "__muldi3", arg1 :: arg2 :: _ -> OK (Some (arg1 * arg2))
  | "__udivdi3", arg1 :: arg2 :: _ -> OK (Some (arg1 / arg2))
  | "__umoddi3", arg1 :: arg2 :: _ -> OK (Some (arg1 mod arg2))
  | "error", _ ->
