...
let rec dump_cfgexpr : expr -> string = function
  | Ebinop (b, e1, e2) ->
      Format.sprintf "(%s %s %s)" (dump_cfgexpr e1) (dump_binop b)
        (dump_cfgexpr e2)
...
let dump_list_cfgexpr l = l |> List.map dump_cfgexpr |> String.concat ", "
...
let dump_cfg_node oc (node : cfg_node) =
  match node with
  | Cstore (e1, e2, sz, _) ->
      Format.fprintf oc "*(%s,%d) = %s" (dump_cfgexpr e1) sz (dump_cfgexpr e2)
  ...
  | Ccall (fname, params, _) ->
      params |> List.map dump_cfgexpr |> String.concat ", "
      |> Format.fprintf oc "%s(%s)" fname
  ...
  Hashtbl.iter
    (fun n node ->
      Format.fprintf oc "n_%s_%d [label=\"%a\" xlabel=\"%d\" shape=%s];\n"
        cfgfunname n dump_cfg_node node n
        (if n = cfgentry then "rectangle peripheries=2" else "rectangle");
